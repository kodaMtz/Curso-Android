...
fun SportsApp(
    ...
    // Determinamos el contentType según el ancho de ventana
    ...
    Scaffold(
        topBar = {
            SportsAppBar(
                ...
                isExpandedScreen = contentType == SportsContentType.ListAndDetail,
                onBackButtonClick = { viewModel.navigateToListPage() },
            )
        }
    ) { innerPadding ->
        if (contentType == SportsContentType.ListAndDetail) {
            // Pantalla expandida: lista y detalle en paralelo
            ...
            // Pantalla compacta: navegación entre pantallas
...
/**
 * Pantalla expandida: lista + detalle lado a lado
 */
@Composable
fun SportsListAndDetails(
    ...
    Row(modifier = modifier) {
        ...
        SportsDetail(
            ...
            contentPadding = contentPadding,
            // En pantalla expandida, Atrás cierra la app
...
fun SportsAppBar(
    ...
    isExpandedScreen: Boolean,        // ← nuevo parámetro
    modifier: Modifier = Modifier
) {
    TopAppBar(
        title = {
            Text(
                text = if (!isShowingListPage && !isExpandedScreen) {
        ...
        navigationIcon = if (!isShowingListPage && !isExpandedScreen) {
            {
...
@OptIn(ExperimentalMaterial3WindowSizeClassApi::class)
@Preview(widthDp = 1000)   // simula pantalla expandida
@Composable
